...
把 LLM 调用写成 Python 函数，返回 Pydantic 实例的 agent 框架。
...
| 聊天体验 | 高 | 低（先填 schema 再渲染） |
| 可测试 / 可审计 / 可回放 | 低（文本语义断言难） | 高（`==` 对 Pydantic 实例） |

**定位**：pyxis 不对标 Codex Desktop 的聊天体验。它适合把 LLM 当作结构化数据生成器：
数据 pipeline 里的 LLM 节点、需要回归测试的业务 agent、多 agent 之间机器对机器协作、
LLM 产出入库/分析的场景。

想做聊天产品，直接用 Anthropic SDK 的 native tool use 更直；
...
- **Codex（我）——AI 产品经理**。负责把方向落到 API 设计、技术选型、
  ROADMAP 取舍和"故意不做"的声明上。骨架设计、commit、文档同步都要自己判断，
  不是只把用户的话翻译成代码。
- **用户——方向引导者 + 品味守门员**。提供喜好、品味和方向。
  用户不写实现、不替 AI 收窄 API，但决定 taste 在哪里收敛。
...
1. 不要把强主张磨平。比如"不做 LangChain-lite"、"不追聊天体验"，
   这些边界是产品判断，不是临时口号。
2. ROADMAP 的"故意不做"、commit message、TODO 骨架里的非目标，都可以继续推进。
   发现判断过期了就改文档和代码，不要靠含糊表述绕开。
3. 用户守方向和品味，AI 负责把实现往前推。需要取舍时，直接给判断。
...
- **code as prompt**：Pydantic 输出模型、字段说明、函数签名和函数体返回的
  输入文本共同定义一次调用。函数 docstring 只用于 Python 文档，
  不进入 LLM 上下文。
- **schema as workflow**：Pydantic 输出模型的字段从上到下生成，字段顺序就是输出顺序。
- **不要重复 schema**：稳定的输出要求、枚举边界、字段顺序
  放进 Pydantic；input builder 只补本次调用独有的材料和少量上下文。不要
  把 response model 再用自然语言复述一遍。
- **函数边界**：pyxis 把一次 LLM 调用写成函数，输入是一段任务说明，输出是
  一个 Pydantic 实例。
...
| 范围 | 机制 | 职责 |
|------|------|------|
| **隐式**（单次 LLM 调用） | `instructor` + Pydantic 字段顺序 | 单次调用内部的输出顺序 |
| **显式**（多次 LLM 调用） | 纯 Python 代码 | 调用**之间**的组合、分支、循环 |

pyxis 不为多步编排发明 DSL。Python 本身就有 `if`、`for`、函数组合。
我们只提供这些概念：
...
- `Tool`：`BaseModel` 子类，带 `run() -> str`。字段是参数，`run()` 执行动作。
...
- `pyxis.mcp` —— MCP 适配层。`MCPServer` + `StdioMCP | HttpMCP`（判别式
  ...
  `[NativeTool1, *stdio_tools, *http_tools]` 直接进判别式联合。**故意不做**：
  `arun` / SSE 传输（老规范的 `GET /sse`
...
- 手写 messages 列表的入口不给——schema 定义返回格式、函数体返回 user message。
...
1. 产品 / 定位层变化，直接改 `docs/concepts/`、`README.md`、`AGENTS.md`。
